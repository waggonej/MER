...
rocketAngle = getRocketStartAngle()
velocityAngle = getVelocityAngle()
velocityAngle += (rocketAngle - 90) # makes angle relative to earth's surface with zero degrees being in the clockwise direction from the point
...
marsangle = 0  # deg
rocketangle = rocketAngle # deg
moondis = 4.066E8  # meters
marsdis = 8.066E8 # meters
#rocketmiledis = 0 # miles
rocketdis = earthRadius + 1 #launch pad relative to center of earth, meters
startTime = 0  # seconds
timeDelta = 1 # seconds
...
positionRocket = np.array([rocketdis * cos(rocketangle),rocketdis * sin(rocketangle),0])
...
V_r = percentorbitalspeed #(G * moonMass / rocketdis) ** (1 / 2.0) * percentorbitalspeed # m/s, orbital speed osf rocket around moon
startingMoonVelocity = np.array([V_m * cos(moonvelocityangle), V_m * sin(moonvelocityangle), 0])  # vector velocity of moon at start
startingRocketVelocity = np.array([V_r * cos(velocityAngle),V_r * sin(velocityAngle),0])
...
#plots earth's curve
surfaceangle = 0
while surfaceangle < 360:
    plt.plot([earthRadius*cos(surfaceangle)], [earthRadius*sin(surfaceangle)], color='blue', linestyle='dashed', marker='o', markerfacecolor='blue', markersize=10) # plots earth
    surfaceangle += 1
